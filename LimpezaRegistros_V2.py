...
with open('producao_alimentos.csv', 'r') as arquivo:
    ...
    conexao.execute('CREATE TABLE producao (PRODUTO TEXT,'\
                    'QTD_PRODUZIDA INTEGER, PRECO_MEDIO REAL, RECEITA_TOTAL REAL)')

    try:
        for linhacsv in leitura:
            if int(linhacsv[1] )> 10:
                linhacsv[3] = retirarPontoValor(linhacsv[3])
                conexao.execute(
                    'INSERT INTO producao (PRODUTO, QTD_PRODUZIDA,PRECO_MEDIO,RECEITA_TOTAL) VALUES(?,?,?,?)', linhacsv)
        conexao.commit()
